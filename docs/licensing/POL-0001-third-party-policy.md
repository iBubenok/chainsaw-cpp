# POL-0001 — Политика сторонних компонентов и данных (Step 4)
...
- upstream-исходники/данные (ART-0004/0005/0006), используемые как **источник истины** поведения;
...
1) Любой сторонний компонент, который **попадает в репозиторий** (код/данные/доки/скрипты/встроенные ресурсы), должен быть отражён в:
   ...
   - отдельном реестре лицензий для зависимостей (будет введён после Step 17, когда появятся выбранные библиотеки).
...
**Проверка:**
- в Step 7/13/27 — сверка наблюдаемого поведения Chainsaw и проверка, что вывод включает достаточную информацию для DRL (не меняя формат 1:1);
...
1) До формализации политики тестовых данных (Step 28) крупные датасеты целиком **не включаются в репозиторий** (хранятся как внешние артефакты).
2) Если по требованиям тестов потребуется добавить в репозиторий минимальные фикстуры (подмножество EVTX):
   - хранить их в выделенном каталоге фикстур (будет определён в Step 28),
...
До выбора библиотек (Step 17) фиксируются только критерии допустимости:
