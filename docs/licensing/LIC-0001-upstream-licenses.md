# LIC-0001 — Реестр лицензий upstream-источников (Step 4)
...
## Объём (Scope)
В рамках Step 4 рассматриваются входные источники:
- **ART-0004**: Chainsaw (Rust) — архив `chainsaw.zip`
- **ART-0005**: Sigma rules — архив `sigma.zip`
- **ART-0006**: EVTX-ATTACK-SAMPLES — архив `EVTX-ATTACK-SAMPLES.zip`

Лицензии будущих C++ зависимостей (будут выбираться в Step 17) **не анализируются** в этом документе.
...
### 3) EVTX-ATTACK-SAMPLES (ART-0006)
...
- Политика включения тестовых данных в репозиторий порта формализуется отдельно в Step 28 (см. RISK-0003).

## Неопределённости (если понадобится уточнение)
1) **SPDX-идентификаторы и режим "only" vs "or-later"** для GPL в upstream-источниках.
   - В архивах присутствует текст GPL v3, однако не все проекты явно маркируют режим `-only`/`-or-later`.
