# BASELINE-0001 — Baseline-версии и происхождение upstream-источников (Step 3)

## Цель
Зафиксировать **что именно портируем** (версии/коммиты/происхождение) для всех входных upstream-источников.

## Объём (Scope)
Фиксируются baseline-метаданные для:
- ART-0004: Chainsaw (Rust)
- ART-0005: Sigma rules
- ART-0006: EVTX-ATTACK-SAMPLES

Не фиксируются (в рамках Step 3):
- наблюдаемое поведение/CLI-контракт (это Step 7+);
- результаты прогонов (это Step 13);
- выбор библиотек/архитектуры C++ (это после завершения As-Is, Gate B→D).

## Метод извлечения baseline (воспроизводимо)
1) Проверка целостности исходных архивов (SHA-256).
...
### Chainsaw (Rust) — ART-0004 (`chainsaw.zip`)
...
### Sigma rules — ART-0005 (`sigma.zip`)
...
### EVTX-ATTACK-SAMPLES — ART-0006 (`EVTX-ATTACK-SAMPLES.zip`)
...
### BL-UNKN-0001 — связь версий submodules Chainsaw ↔ архивы `sigma.zip`/`EVTX-ATTACK-SAMPLES.zip`
Наблюдения:
...
Следствие:
- Невозможно доказательно установить «ожидаемые» ревизии submodules, т.е. нельзя формально утверждать, что архивы `sigma.zip` и `EVTX-ATTACK-SAMPLES.zip` соответствуют конкретной синхронизации upstream Chainsaw.

Принятое действие:
- Оставить как риск RISK-0006 до получения дополнительных доказательств (см. `00_RISK_REGISTER.md`).
...
> Ниже приведён минимальный набор команд для воспроизведения извлечения baseline из исходных архивов.
