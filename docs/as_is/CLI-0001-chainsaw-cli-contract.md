# CLI-0001 — CLI-контракт Chainsaw (Rust) As‑Is (Step 7)
...
Не фиксируются (в рамках Step 7):
- внутренняя архитектура Rust и потоки данных (Step 9);
- детальные спецификации форматов входных данных и трансформаций (Step 10);
- перечень фич продукта целиком (Step 8);
- любые To‑Be решения/выбор библиотек (после завершения As‑Is и Gate B→D).

## Baseline
Baseline upstream‑версии зафиксирован в документе `BASELINE-0001` (Step 3). Этот CLI‑контракт относится к тому же baseline (`ART-0004`).

## Источники (primary)
- `ART-0004/src/main.rs` — определение CLI через `clap` (Parser/Subcommand) и реализация команд в `run()` + обработка ошибок/exit code в `main()`.
- `ART-0004/src/write.rs` — макросы вывода (`cs_*`), разделение stdout/stderr, поведение `quiet` и `verbose`, запись в файл через `--output`.
- `ART-0004/src/cli.rs` — platform‑dependent детали интерфейса (префиксы/спиннер/прогрессбар).
- `ART-0004/src/file/mod.rs` — рекурсивный сбор файлов и фильтрация по расширениям.
- `ART-0004/src/rule/mod.rs` — допустимые значения для `--kind/--level/--status` и сообщения об ошибках парсинга.
- `ART-0004/tests/clo.rs` + ожидаемые файлы `ART-0004/tests/**.txt` — примеры реальных CLI‑вызовов и подтверждение успешного exit‑кода для отдельных сценариев.

## Источники (secondary)
- `ART-0004/README.md` — описания/примеры использования. Используется только как справка; при противоречии приоритет у кода.
...
Источник: `ART-0004/src/main.rs:24-56`.
...
Источник: `ART-0004/src/main.rs:44-56`, применение `num_threads`: `ART-0004/src/main.rs:401-407`.
...
Источник:
- вывод в stdout/файл: `ART-0004/src/write.rs:63-103`, `132-183`;
- подавление stderr при `quiet`: `ART-0004/src/write.rs:123-130`.
...
Источник: `ART-0004/src/main.rs:356-399`.
...
Источник: логика `csv => (Some(path), None)` в `init_writer`: `ART-0004/src/main.rs:356-389`.
...
Источник: `ART-0004/src/write.rs:105-130`.
...
Источник: `ART-0004/src/cli.rs:25-70`.
...
Источник: `ART-0004/src/main.rs:1152-1160`.

Формат:
- если есть `cause` (вторая ошибка в цепочке): `[x] <err> - <cause>`
- иначе: `[x] <err>`

Источник: `ART-0004/src/main.rs:1153-1158`.

### 1.6.2. Успех
Для проверенных CLI‑сценариев (тесты Rust) код завершения = `0`.

Источник: `ART-0004/tests/clo.rs:6-145` (используется `.success()`).
...
Источник: описание и ветка `Command::Dump`: `ART-0004/src/main.rs:58-94`, `401-519`.

### Аргументы
- `path: Vec<PathBuf>` — один или несколько путей (файлы/директории).

Источник: `ART-0004/src/main.rs:60-64`.
...
Источник: `ART-0004/src/main.rs:60-94`.

### Фильтрация по расширению
- `--extension` сравнивается со значением `Path::extension()` (без точки), т.е. ожидается ввод вида `evtx`, `json`, `yaml` и т.п.

Источник: формирование `HashSet::from([extension])`: `ART-0004/src/main.rs:441-444`; сравнение по `path.extension()`: `ART-0004/src/file/mod.rs:479-488`.
...
Источник: `ART-0004/src/main.rs:435-519`.
...
Источник: `ART-0004/src/main.rs:422-519`.
...
Источник: `ART-0004/src/main.rs:356-399`, `452-455`.
...
Источник: описание и ветка `Command::Hunt`: `ART-0004/src/main.rs:96-189`, `520-810`.

### Аргументы
- `rules: Option<PathBuf>` — путь к коллекции правил (опционально).
- `path: Vec<PathBuf>` — пути к артефактам для обработки.

Источник: `ART-0004/src/main.rs:96-103`.
...
Источник: `ART-0004/src/main.rs:570-579`.
...
Источник: `ART-0004/src/main.rs:104-188`; допустимые значения: `ART-0004/src/rule/mod.rs:105-205`.
...
Источник: `ART-0004/src/main.rs:125-188`.
...
Источник: `ART-0004/src/main.rs:111-168`.
...
Источник: описание и ветка `Command::Lint`: `ART-0004/src/main.rs:191-201`, `811-872`.

### Аргументы
- `path: PathBuf` — путь к коллекции правил.

Источник: `ART-0004/src/main.rs:191-200`.

### Опции
- `--kind <chainsaw|sigma>` — тип правил для линтинга.
- `-t, --tau` — вывести tau‑представление.

Источник: `ART-0004/src/main.rs:191-201`; допустимые значения kind: `ART-0004/src/rule/mod.rs:127-137`.
...
Источник: `ART-0004/src/main.rs:816-871`.
...
Источник: описание и ветка `Command::Search`: `ART-0004/src/main.rs:203-270`, `873-1043`.
...
Источник: `ART-0004/src/main.rs:205-212`, дефолт current dir: `ART-0004/src/main.rs:909-913`.
...
Источник: `ART-0004/src/main.rs:213-269`.
...
Источник: `ART-0004/src/main.rs:279-305`, `1046-1089`.

### Аргументы
- `<shimcache>` — путь к SYSTEM hive.

Источник: `ART-0004/src/main.rs:283-285`.
...
Источник: `ART-0004/src/main.rs:286-305`.
...
Источник: `ART-0004/src/main.rs:1057-1088`.
...
Источник: `ART-0004/src/main.rs:306-322`, `1090-1145`.

### Аргументы
- `<srum_path>` — путь к SRUDB.dat.
- `-s, --software <path>` — путь к SOFTWARE hive (**обязателен**).

Источник: `ART-0004/src/main.rs:308-313`.
...
Источник: `ART-0004/src/main.rs:313-322`.
...
- **CLI-UNKN-0001 — точные тексты `--help`/`--version` и exit code при ошибках парсинга аргументов.**
  - Причина: в типовой CI-среде отсутствует Rust toolchain (`cargo`), невозможно воспроизвести `chainsaw --help`.
  - План подтверждения: Step 13 golden runs — снять `--help/--version` и сценарии ошибок (неизвестная команда/флаг, missing required arg) с фиксацией stdout/stderr/exit code.
...
- **CLI-UNKN-0002 — минимальное число значений для позиционных `Vec<PathBuf>` (например, `dump <path...>` / `hunt <path...>`).**
  ...
  - План подтверждения: Step 13 — запуски с отсутствующими позиционными аргументами и фиксация поведения.
...
- **CLI-UNKN-0003 — точная семантика `ArgGroup` (`group = "format"`) для взаимного исключения форматов.**
  ...
  - План подтверждения: Step 13 — попытка указать взаимоисключающие комбинации (`--json --csv`, `--json --jsonl` и т.д.) и фиксация ошибки/exit code.
...
- **CLI-UNKN-0004 — exit code при `analyse srum` в случае ошибки парсинга SRUM.**
  ...
  - План подтверждения: Step 13 — запустить `analyse srum` на заведомо некорректном входе и зафиксировать exit code.

- **CLI-UNKN-0005 — детерминизм порядка результатов `search` при множестве файлов.**
  - Наблюдение: используется `files.par_iter()`.
  - План подтверждения: Step 13 — повторные прогоны на каталоге с несколькими файлами и сравнение порядка результатов.

---

# Приложение A — runbook для подтверждения (Step 13)
