...
Ограничение среды: на Step 8 не выполнялись реальные прогоны бинаря; наблюдаемое поведение фиксируется по тестам Rust и по коду CLI/логики. Сценарии, требующие подтверждения запуском, помечены как **UNKNOWN** и выносятся в план Step 13.
...
1) `lint --kind stalker` упомянут в help‑тексте подкоманды `lint`, но в `RuleKind` отсутствует значение `stalker`.
   ...
   - План верификации: на Step 13 зафиксировать вывод `chainsaw lint --help` + попытку `chainsaw lint --kind stalker ...` и ожидаемую ошибку/валидацию clap.
