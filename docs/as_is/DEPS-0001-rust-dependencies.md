...
- Single Source of Truth для As‑Is карты зависимостей на фазе As‑Is (Step 11).
...
1) `hex` присутствует в runtime‑зависимостях, но прямых мест использования в `src/` и `tests/` на текущем срезе не найдено.
   - План верификации: при Step 12/13 (инвентаризация тестов / golden runs) проверить, не задействуется ли `hex` в опциональных ветках CLI или через feature‑flags; дополнительно проверить `cargo tree -e features` при наличии toolchain.
