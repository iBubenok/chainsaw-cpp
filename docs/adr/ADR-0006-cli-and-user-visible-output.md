...
- Потребуется отдельная тестовая база на CLI‑поверхность и пользовательские форматы (частично уже покрыто `RUN-0001…RUN-0104` и `TST-0001…TST-0006`).

## План верификации
- Step 24: CLI‑поверхность скелета должна соответствовать `CLI-0001` по командам/флагам и базовым кодам возврата (заглушки допустимы, но должны быть оформлены).
- Step 27/33: дифф‑сравнение по `RUN-0001…RUN-0104` (help/version/ошибки), а также сценариям, содержащим табличный вывод и JSON/JSONL.
