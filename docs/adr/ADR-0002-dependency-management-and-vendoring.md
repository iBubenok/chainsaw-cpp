...
- Для CI и для air‑gapped окружений базовый режим без сети снижает операционные риски и соответствует требованиям безопасности (`REQ-SEC-0019`).
...
## Последствия
- Репозиторий может вырасти по размеру (компенсируется политикой данных Step 28 и строгими правилами исключения мусора `REQ-OPS-0029`).
...
## План верификации
- Step 24: C++‑скелет должен собираться из «чистого» состояния без сети, используя только содержимое репозитория.
- Step 26: CI матрица подтверждает воспроизводимость сборки на Windows/Linux/macOS.
- Step 43: аудит лицензий подтверждает наличие текстов лицензий/NOTICE и корректность записей в реестрах.
