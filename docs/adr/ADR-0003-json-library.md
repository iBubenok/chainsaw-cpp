...
3) Безопасность и устойчивость:
   ...
   - парсер должен иметь ограничения по глубине/размеру; конкретные лимиты фиксируются в реализации и в developer guide (Step 21).
...
## План верификации
- Step 27/33: дифф‑сравнение JSON/JSONL выводов по сценариям `RUN-020*`, `RUN-030*`, `RUN-040*`, `RUN-050*` согласно `GOV-0002`.
- Step 32: перенос/эквивалентные тесты `TST-*`, которые завязаны на JSON структуры (см. `TESTMAT-0001`).
