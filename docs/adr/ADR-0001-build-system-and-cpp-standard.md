...
На этапе выбора базовых решений (Step 18) доступны:
...
- ограничения среды CI как основной способ получения доказательств (см. `RISK-0021`).
...
- Минусы: неоднородность поддержки на целевых платформах/стандартных библиотеках может увеличить риск сборки и усложнить “минимально собираемый скелет” (Step 24).

## Обоснование
- CMake является наиболее распространённым и ожидаемым инструментом в кроссплатформенном C++‑разработке; упрощает дальнейшее подключение CI матрицы (Step 26) и внешних зависимостей.
...
## Последствия
- Все последующие решения по зависимостям и инфраструктуре (Step 24–26) должны быть совместимы с CMake и C++17.
...
## План верификации
- Step 24: минимальный C++‑скелет должен успешно конфигурироваться и собираться хотя бы в одной целевой среде.
- Step 26: CI‑матрица должна подтверждать сборку/тесты на Windows/Linux/macOS.
