# TASK-0001 — Рабочая постановка задачи: портирование Chainsaw (Rust) → C++ (Step 15)
...
- Риски/неясности: `00_RISK_REGISTER.md` (ключевые: RISK-0003, RISK-0006, RISK-0008, RISK-0009, RISK-0011, RISK-0022, RISK-0028, RISK-0029)
...
3) Оптимизации производительности до достижения функционального паритета и доказательства 1:1 (оптимизации допускаются только после parity и через harness; см. план Step 37–38).
...
5) Продуктовая упаковка/дистрибуция (как финальная поставка) — предмет отдельных шагов плана (Step 40+), но требования к ней будут сформулированы в AR (Step 16).
...
- CLI meta/help/version и ошибки CLI: `RUN-0001…RUN-0104`.
- Поиск по EVTX: `RUN-0201…RUN-0204`.
- Hunt по rules: `RUN-0301…RUN-0303`.
- Dump (JSON/Std): `RUN-0401…RUN-0402`.
- Analyse SRUM: `RUN-0501…RUN-0502`.
- Lint Sigma: `RUN-0601`.
...
|---|---|---|---|
| Глобальные флаги (`--no-banner`, `--num-threads`, `-v`) | FEAT-0001 | (нет upstream тестов) | RUN-0001…RUN-0003 (+ учитывать RISK-0028) |
| `search` | FEAT-0007, FEAT-0008, FEAT-0009 | TST-0001…TST-0003 | RUN-0201…RUN-0204 |
| `hunt` | FEAT-0003, FEAT-0004, FEAT-0008, FEAT-0009 | TST-0004 | RUN-0301…RUN-0303 |
| `dump` | FEAT-0002, FEAT-0008, FEAT-0009 | (нет upstream тестов) | RUN-0401…RUN-0402 |
| `lint` | FEAT-0006 | (нет upstream тестов) | RUN-0103…RUN-0104, RUN-0601 |
| `analyse srum` | FEAT-0011 | TST-0005…TST-0006 | RUN-0501…RUN-0502 |
...
1) **Код порта (C++)**
   ...
   - структура/границы/интерфейсы будут зафиксированы в To‑Be архитектуре (Step 19), но на уровне постановки требуется, чтобы границы позволяли трассировку к `FEAT-*`/`TST-*`.

2) **Доказательная инфраструктура 1:1**
   - перенос тестов/эквивалентные тесты для `TST-*` (Step 32);
   - harness дифф‑сравнения Rust vs C++ по `RUN-*` (Step 27/33);
   - отчёты `REP-*` по результатам проверок.

3) **Кроссплатформенный цикл сборки/проверки**
   - воспроизводимый configure/build/test цикл (локально и в CI) для Windows/Linux/macOS (Step 24–26);
   - правила качества и «зелёного состояния» (Step 25).

4) **Документация и управляемость**
   - AR (Step 16), ADR (Step 18), To‑Be (Step 19), WBS (Step 20), Developer guide (Step 21);
   - реестры `00_*` обновляются на каждом шаге;
   - политика тестовых данных (Step 28), учитывающая лицензии и воспроизводимость.
...
4) **Документация согласована и трассируемость замкнута:**
   - пройден финальный аудит консистентности (Step 42);
...
- RISK-0003: политика тестовых данных (размер/лицензии/воспроизводимость) — закрывается Step 28.
- RISK-0006: синхронизация Chainsaw ↔ Sigma/EVTX — закрывается Step 28 (baseline/доказательства) либо принимается как ограничение.
...
- E2E сценарии ↔ `REP-0001` (`RUN-*`) + архивы `rust_golden_runs_*` (ART-0038…ART-0040).
- Определение 1:1 ↔ `GOV-0002` (C1–C7).
- Неясности/ограничения ↔ `00_RISK_REGISTER.md` (RISK-*).
