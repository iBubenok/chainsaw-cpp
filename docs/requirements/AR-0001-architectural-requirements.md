# AR-0001 — Архитектурные требования (FR/NFR/SEC/OPS) для порта Chainsaw на C++ (Step 16)
...
- Контекст ИБ‑домена: `docs/context/Ambrella_company_profile.docx` (ART-0003)
- Риски/допущения (SoT неопределённостей): `00_RISK_REGISTER.md`
...
**Критерии приёмки:**
- Для всех CLI‑сценариев `RUN-0001…RUN-0104` из `REP-0001` наблюдаемое поведение совпадает с Rust по критериям `GOV-0002` (C1–C3, C6).

**Верификация:**
- diff‑harness по `RUN-0001…RUN-0104` (Step 27/33);
- дополнительные test‑to‑test проверки, если потребуются для аргпарсинга (Step 32).
...
- Для `RUN-0201…RUN-0204` (см. `REP-0001`) stdout/stderr/exit code совпадают с Rust по `GOV-0002`.

**Верификация:**
- test‑to‑test (Step 32);
- diff‑harness по `RUN-0201…RUN-0204` (Step 27/33).
...
- Для `RUN-0301…RUN-0303` stdout/stderr/exit code и побочные артефакты совпадают с Rust по `GOV-0002` (включая C4/C5).

**Верификация:**
- test‑to‑test (Step 32);
- diff‑harness по `RUN-0301…RUN-0303` (Step 27/33).
...
**Критерии приёмки:**
- Для `RUN-0401…RUN-0402` stdout/stderr/exit code совпадают с Rust по `GOV-0002`.

**Верификация:**
- diff‑harness по `RUN-0401…RUN-0402` (Step 27/33);
- при необходимости — добавление test‑to‑test проверок на сериализацию (Step 32).
...
**Критерии приёмки:**
- Для `RUN-0103…RUN-0104` и `RUN-0601` stdout/stderr/exit code совпадают с Rust по `GOV-0002`.

**Верификация:**
- diff‑harness по `RUN-0103…RUN-0104`, `RUN-0601` (Step 27/33).
...
- Для `RUN-0501…RUN-0502` stdout/stderr/exit code и выходные файлы (если задействованы) совпадают с Rust по `GOV-0002`.

**Верификация:**
- test‑to‑test (Step 32);
- diff‑harness по `RUN-0501…RUN-0502` (Step 27/33).
...
**Критерии приёмки:**
- Добавлены и зафиксированы разрешённые фикстуры (SYSTEM hive, опционально Amcache) согласно политике данных (Step 28).
...
**Верификация:**
- новые `RUN-*` + diff‑harness (Step 27/33);
- или test‑to‑test (Step 32) при наличии upstream‑тестов/эквивалентных проверок.
...
- Дополнительно, `RUN-0601` (`lint --kind sigma -t`) совпадает по `GOV-0002`.

**Верификация:**
- test‑to‑test для `TST-0007…TST-0022` (Step 32);
- diff‑harness по `RUN-0601` (Step 27/33).
...
**Верификация:**
- расширение набора `RUN-*` (Step 27/28) или test‑to‑test (Step 32) согласно `TESTMAT-0001` и `DATA-0001`.
...
**Верификация:**
- test‑to‑test (Step 32/33);
- golden runs + diff‑harness (Step 27/33);
- кроссплатформенный e2e‑валидатор (Step 41).
...
**Критерии приёмки:**
- Для `RUN-0201…RUN-0203` (repeats=3) и иных сценариев, определённых в harness, сравнение повторов не выявляет расхождений stdout/stderr/exit code/файлов.

**Верификация:**
- diff‑harness с повторами и отчётами (Step 27/33).
...
- Для каждой ОС выполняется сравнение с соответствующими Rust golden runs (ART-0038..ART-0040) «внутри платформы».

**Верификация:**
- CI‑матрица (Step 26) + отчёты `REP-*` (Step 33/41).
...
**Критерии приёмки:**
- Сценарии `RUN-0501…RUN-0502`, `RUN-0201…RUN-0204`, `RUN-0301…RUN-0303` выполняются успешно (без аварийного завершения) и совпадают с Rust по `GOV-0002`.

**Верификация:**
- diff‑harness по соответствующим `RUN-*` (Step 27/33).
...
**Критерии приёмки:**
- В `00_TRACEABILITY.md` и в To‑Be архитектуре (Step 19) представлены `MOD-*` границы и связи, замыкающие трассировку минимум для ключевых CLI‑поверхностей (`search/hunt/dump/analyse srum/lint`).

**Верификация:**
- аудит трассируемости (Step 42);
- ревью архитектуры (Step 19).
...
**Верификация:**
- diff‑harness (Step 27/33);
- аудит решений и изменений (Step 42).
...
**Критерии приёмки:**
- Негативные сценарии из `REP-0001` (`RUN-0101`, `RUN-0102`, `RUN-0003`) завершаются с теми же exit codes и сообщениями, что Rust (в пределах платформы).
...
**Верификация:**
- negative golden runs + diff‑harness (Step 27/33);
- fuzz/robustness прогон (Step 35–36) с отчётом `REP-*`.

**Источники/обоснование:** `TASK-0001/1`, `GOV-0002/C6`, ИБ‑контекст (ART-0003).
...
**Критерии приёмки:**
- Для сценариев с файловыми эффектами в `RUN-*` (например, `RUN-0303`) дифф‑harness фиксирует совпадение создаваемых артефактов.
...
**Верификация:**
- diff‑harness (Step 27/33) с сохранением `out_files/`;
...
**Верификация:**
- архитектурный и security‑ревью (Step 39/42);
- анализ зависимостей (Step 17) и их назначений.

**Источники/обоснование:** `TASK-0001/2.2` (non‑goals: не добавлять новые возможности), ИБ‑контекст (ART-0003).
...
**Критерии приёмки:**
- Сценарий `RUN-0303` совпадает по результатам и по побочным файлам с Rust.

**Верификация:**
- diff‑harness по `RUN-0303` (Step 27/33);
- дополнительные негативные сценарии (Step 27/28) при выявлении расхождений.
...
**Верификация:**
- unit/security тесты на генерацию путей (Step 32/39).

**Источники/обоснование:** ИБ‑контекст (ART-0003), `DATA-0001/2.7` (генерация имён), `RISK-0025`.
...
**Верификация:**
- инфраструктура качества (Step 25) + security testing (Step 39) + отчёты `REP-*`.

**Источники/обоснование:** ИБ‑контекст (ART-0003), `TASK-0001/1`.
...
**Критерии приёмки:**
- Developer guide (Step 21) содержит однозначные команды сборки/тестов.
- В CI (Step 26) этот цикл выполняется автоматически и выдаёт артефакты отчётов.

**Верификация:**
- CI логи (Step 26);
- audit документации (Step 42).

**Источники/обоснование:** `TASK-0001/4.3`, `RISK-0021` (ограничения среды CI, требуется внешний воспроизводимый цикл).
...
**Критерии приёмки:**
- Для каждого `RUN-*` существует отчёт `REP-*` с результатом сравнения, и артефакты (stdout/stderr/exit code/out_files + env/build) доступны как внешние артефакты CI.

**Верификация:**
- Step 27/33/41 (harness и отчёты).
...
**Критерии приёмки:**
- Выполнен Step 28: политика оформлена документом и применена к фикстурам/датасетам.

**Верификация:**
- аудит политики и структуры репозитория (Step 28/42).
...
- Для Sigma/EVTX baseline зафиксированы версии/commit/tag и связь с доказательствами (Step 28).

**Верификация:**
- Step 28 (baseline данных) + аудит (Step 42).
...
**Верификация:**
- ревью документации (Step 21/42);
- практический прогон в изолированной среде (по мере доступности).

**Источники/обоснование:** ИБ‑контекст (ART-0003), `RISK-0021`, `POL-0001`.

---

### REQ-OPS-0029 — Управление локальным состоянием и чистота артефактов
**Требование:** Репозиторий и снапшоты не должны содержать локальные/временные данные сборки/прогонов (например, `.local/`, `build/`, `out/`, `inputs/tmp/`).

**Критерии приёмки:**
- Протокол упаковки снапшотов соблюдается; каталоги‑исключения отсутствуют в zip.

**Верификация:**
- ревью содержимого снапшотов (Step 42) + `.gitignore`.

**Источники/обоснование:** `DEC-0008`, правила упаковки снапшотов.

---

### REQ-OPS-0030 — Наличие runbook для внешних прогонов
**Требование:** Для всех проверок, которые нельзя выполнять в типовой CI-среде, должен существовать runbook, позволяющий воспроизводимо получить артефакты и загрузить их как внешние артефакты.

**Критерии приёмки:**
- Для Rust golden runs runbook уже зафиксирован (`REP-0001`, `tools/golden_runs_full/README.md`).
- Для C++ golden runs и diff‑harness будет зафиксирован аналогичный runbook (Step 27).
