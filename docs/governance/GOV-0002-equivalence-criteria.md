# GOV-0002 — Критерии эквивалентности 1:1 и доказательная база приёмки (Step 6)
...
Документ определяет **критерии сравнения** и **правила доказательства**, но не подменяет собой As‑Is спецификации Rust (Step 7–14) и не делает To‑Be решений.
...
**Доказательства:**
- As‑Is спецификация CLI (Step 7) + тесты на парсинг/валидацию (Step 12/32) + golden runs для `--help/--version` (Step 13/27).
...
**Доказательства:**
- golden runs (Step 13) с сохранением сырых stdout/stderr + дифф‑отчёт (Step 27/33).
...
**Доказательства:**
- протоколы golden runs (Step 13) и/или тесты (Step 12/32) + дифф‑отчёты (Step 27/33).
...
**Доказательства:**
- эталонные прогоны Rust на фиксированных датасетах (Step 13) + дифф‑сравнение (Step 27/33);
- test‑to‑test проверки логики (Step 12/32).
...
**Доказательства:**
- golden runs (Step 13) с сохранением артефактов + дифф‑отчёты (Step 27/33).
...
**Доказательства:**
- negative golden runs (Step 13) + тесты на ошибки (Step 12/32) + дифф‑отчёты (Step 27/33).
...
**Критерий:**
- для каждого выбранного сценария (минимум: ключевые e2e сценарии из Step 13/27) результаты Rust повторяемы;
...
> Производительность/оптимизации не входят в критерии 1:1 и рассматриваются отдельно (Step 37–38).
...
1) найдено и доказано (на фактах Step 7/13), что Rust выводит нестабильные/окружение‑зависимые поля;
...
1) **As‑Is спецификации Rust** (Step 7–14)
   - CLI‑контракт,
   - реестр фич,
   - As‑Is архитектура,
   - форматы данных,
...
5) **Кроссплатформенные подтверждения**
   - CI‑логи сборки/тестов по матрице ОС (Step 26),
   - e2e‑валидатор по платформам (Step 41).

### 4.2. Минимальный набор для приёмки слайса (Step 31–33)
...
- выполнен parity‑аудит (Step 34);
- нет регрессий относительно golden runs (Step 13) по выбранному покрытию;
- подтверждена кроссплатформенная сборка/тесты/е2e (Step 26/41) или оформлены исключения как риски.
