# GOV-0003 — Пакет согласования архитектуры и критерии «готово к старту реализации» (Step 22)

## Назначение
Этот документ задаёт **формальный, воспроизводимый пакет согласования** для завершения фазы To‑Be архитектуры (Step 15–22) и подготовки к старту фазы реализации (Step 24–28).
...
3) **критериев “готово к старту реализации”** (какие условия должны быть выполнены перед началом Step 24).
...
- ADR baseline (ключевые решения): `ADR-0001...ADR-0011` — `docs/adr/*` + индекс `00_DECISION_LOG.md`
...
- Реестр артефактов: `00_ARTIFACT_REGISTRY.md`
- Трассируемость (минимум): `00_TRACEABILITY.md`
...
| 3 | Наличие ADR baseline | Все `ADR-0001...` присутствуют, перечислены в `00_DECISION_LOG.md` | `docs/adr/*`, `00_DECISION_LOG.md` |  |  |
...
| 8 | Реестры проекта | `00_PROJECT_STATE.md`, `00_ARTIFACT_REGISTRY.md`, `00_RISK_REGISTER.md`, `00_TRACEABILITY.md` актуальны | файлы в корне репозитория |  |  |
...
| 13 | Golden runs и протокол | Есть протокол golden runs и внешние артефакты результатов | `REQ-OPS-0024`, `REQ-OPS-0030` | `docs/reports/REP-0001-rust-golden-runs.md` + внешние артефакты CI |  |  |
| 14 | Трассируемость определена | Схема `GOV-0001` и минимум `00_TRACEABILITY.md` позволяют связать `REQ-* → ADR/TOBE → MOD → TST/RUN` | `REQ-NFR-0014` | `docs/governance/GOV-0001-*.md`, `00_TRACEABILITY.md` |  |  |
...
| № | Проверка | Критерий PASS | Связанные `REQ-*` | Доказательство (ссылка) | Статус | Комментарий / Action |
|---:|---|---|---|---|---|---|
| 19 | Все ADR индексированы | `00_DECISION_LOG.md` перечисляет `ADR-0001...` со статусами | `REQ-OPS-0023`, `REQ-OPS-0024` | `00_DECISION_LOG.md` |  |  |
...
| 21 | Pending‑решения не блокируют старт Step 24 | Для pending‑тем указано, на каком шаге они становятся блокерами и какой вход нужен | `REQ-OPS-0024` | `ADR-0009` + `WBS-0001` |  |  |

### 1.5. Риски/допущения и безопасность (ИБ‑домен)

| № | Проверка | Критерий PASS | Связанные `REQ-*` | Доказательство (ссылка) | Статус | Комментарий / Action |
|---:|---|---|---|---|---|---|
| 22 | Риски актуальны | У каждого `RISK-*` есть влияние/приоритет/план закрытия или статус Accepted | `REQ-OPS-0024` | `00_RISK_REGISTER.md` |  |  |
| 23 | Нет P1 без плана | Нет открытых P1‑рисков без конкретного плана закрытия | `REQ-OPS-0024` | `00_RISK_REGISTER.md` |  |  |
...
| 28 | Политика тестовых данных | Определено, что хранится в репо, что остаётся внешним и как обеспечить воспроизводимость | `REQ-OPS-0025`, `REQ-OPS-0027` | (будет финализировано на Step 28; промежуточные правила — в `POL-0001`) |  |  |
...
## 2) Критерии «готово к старту реализации» (перед Step 24)

### 2.1. MUST (без этого старт реализации запрещён)
1) Пройден чек‑лист раздела 1 с результатом:
   - **PASS**, или
   - **PASS-with-risks** (при этом список рисков/исключений зафиксирован в протоколе, и они не блокируют Step 24).
...
4) Нет открытых P1‑рисков без плана закрытия (`00_RISK_REGISTER.md`).
5) Pending‑решения (например, по парсерам из `ADR-0009`) **явно перечислены** и в протоколе указано:
   - почему они не блокируют Step 24,
   - на каком шаге станут блокерами,
   - какой вход/эксперимент нужен для закрытия.

### 2.2. SHOULD (желательно до Step 26/27, но не блокирует Step 24)
- Имеются предварительные наброски структуры будущего diff‑harness и CI‑матрицы (будет реализовано на Step 26–27).
- Уточнён минимальный набор тестовых данных, подлежащий хранению/внешнему размещению (будет формализовано на Step 28).

### 2.3. Допустимые открытые вопросы при старте реализации (если зафиксированы)
- Выбор конкретных EVTX/HVE/MFT/XML парсеров (pending в `ADR-0009`) — допускается как открытый вопрос на Step 24–25, если реализация идёт через абстракции `MOD-*` и не требует конкретного парсера на уровне скелета.
...
## Контекст
- Основание: Step 22 (пакет согласования архитектуры), подготовка к Gate D→E (после Step 23)
- Объект согласования: To‑Be документы Step 15–22
...
- Реестры: `00_PROJECT_STATE.md`, `00_ARTIFACT_REGISTRY.md`, `00_RISK_REGISTER.md`, `00_TRACEABILITY.md`
