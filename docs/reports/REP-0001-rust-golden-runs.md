# REP-0001 — Эталонные прогоны Chainsaw (Rust) (golden runs): протокол и результаты (Step 13)
...
- автоматического **дифф‑сравнения** с C++ портом (Step 27/33);
...
На момент подготовки протокола запуск upstream Chainsaw в типовой CI-среде **невозможен** из‑за отсутствия Rust toolchain (`cargo`/`rustc`).

Факт подтверждения см. в Step 13 (репорт итерации) и в `RISK-0021`.
...
- **RUN-0001**: `chainsaw --help`
- **RUN-0002**: `chainsaw --version`
- **RUN-0003**: `chainsaw` (без аргументов)

### 4.2. CLI ошибки (clap/валидация)
- **RUN-0101**: `chainsaw no_such_cmd`
- **RUN-0102**: `chainsaw dump` (ожидается ошибка отсутствия обязательного `path`)
- **RUN-0103**: `chainsaw lint --help`
- **RUN-0104**: `chainsaw lint --kind stalker <chainsaw_rules_dir>` (проверка RISK-0013)

### 4.3. `search` (проверка C2/C3/C7 + подготовка дифф‑harness)
- **RUN-0201**: `chainsaw search 4624 <security_sample.evtx> -q` (repeats: 3)
- **RUN-0202**: `chainsaw search 4624 <security_sample.evtx> -jq` (repeats: 3)
- **RUN-0203**: `chainsaw search 4624 <security_sample.evtx> -q --jsonl` (repeats: 3)
- **RUN-0204**: `chainsaw --num-threads 1 search 4624 <EVTX-ATTACK-SAMPLES/Command and Control/DE_RDP_Tunneling_4624.evtx> -q --jsonl` (проверка путей с пробелами)

### 4.4. `hunt` (проверка C2/C3/C4/C5)
- **RUN-0301**: `chainsaw hunt <security_sample.evtx> -r <rule-any-logon.yml>`
- **RUN-0302**: `chainsaw hunt <security_sample.evtx> -r <rule-any-logon.yml> --jsonl -q`
- **RUN-0303**: `chainsaw hunt <security_sample.evtx> -r <rule-any-logon.yml> --jsonl --cache-to-disk -q` (проверка RISK-0014)

### 4.5. `dump` (проверка C2/C3/C5)
- **RUN-0401**: `chainsaw dump <security_sample.evtx> -q`
- **RUN-0402**: `chainsaw dump <security_sample.evtx> -q --json`

### 4.6. `analyse srum` (проверка C2/C3/C5)
- **RUN-0501**: `chainsaw analyse srum --software <SOFTWARE> <SRUDB.dat> --stats-only -q`
- **RUN-0502**: `chainsaw analyse srum --software <SOFTWARE> <SRUDB.dat> -q`

### 4.7. `lint` для Sigma (проверка конвертации/taulogic)
- **RUN-0601**: `chainsaw lint --kind sigma -t <sigma_rule_file>`

> Примечание: сценарии для `analyse shimcache` не включены из-за отсутствия входных данных (SYSTEM hive) в доступных материалах. См. `RISK-0022`.
...
Сохранить архив как внешний артефакт CI.
...
|---|---|---:|---:|---|
| RUN-0001 | OK | 0 | 1 | chainsaw --help (linux x86_64) |
| RUN-0002 | OK | 0 | 1 | chainsaw --version (linux x86_64) |
| RUN-0003 | OK (ожидаемая ошибка) | 2 | 1 | chainsaw без аргументов → clap usage |
| RUN-0101 | OK (ожидаемая ошибка) | 2 | 1 | неизвестная команда |
| RUN-0102 | OK (ожидаемая ошибка) | 1 | 1 | dump без path |
| RUN-0103 | OK | 0 | 1 | lint --help |
| RUN-0104 | OK (ожидаемая ошибка) | 2 | 1 | lint --kind stalker |
| RUN-0201 | OK | 0 | 3 | search 4624 yaml quiet |
| RUN-0202 | OK | 0 | 3 | search 4624 json quiet |
| RUN-0203 | OK | 0 | 3 | search 4624 jsonl quiet |
| RUN-0204 | OK | 0 | 1 | search 4624 (EVTX-ATTACK-SAMPLES path with spaces) |
| RUN-0301 | OK | 0 | 1 | hunt table |
| RUN-0302 | OK | 0 | 1 | hunt --jsonl -q |
| RUN-0303 | OK | 0 | 1 | hunt --jsonl --cache-to-disk -q |
| RUN-0401 | OK | 0 | 1 | dump yaml -q |
| RUN-0402 | OK | 0 | 1 | dump json -q |
| RUN-0501 | OK | 0 | 1 | analyse srum --stats-only -q |
| RUN-0502 | OK | 0 | 1 | analyse srum full -q |
| RUN-0601 | OK | 0 | 1 | lint --kind sigma -t win_esent_ntdsutil_abuse.yml |
...
- Windows/macOS архивы подготовлены аналогично (SHA-256 см. сопроводительную документацию); структура ожидается согласно разделу 3.
