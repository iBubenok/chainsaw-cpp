# WBS-0001 — Work Breakdown Structure и оценка трудозатрат (порт Chainsaw Rust → C++) (Step 20)
...
- Риски/допущения `RISK-*`: `00_RISK_REGISTER.md`.
...
### 1.1. Блок D (Step 21–23) — доведение пакета To‑Be до формального старта реализации

#### WBS-1.1 — Step 21: Практическое руководство разработчика
- Результат: developer guide (стандарты кода/тестов/безопасности/кроссплатформенности), согласованный с AR/ADR/To‑Be.
- Зависимости: завершённые Step 16/18/19.
- Оценка: **3 ч/д** (Medium).
- Трассируемость:
...
#### WBS-1.2 — Step 22: Пакет согласования архитектуры
- Результат: checklist/шаблон протокола согласования старта реализации; критерии «готово к Step 24».
...
#### WBS-1.3 — Step 23: Аудит консистентности документов №1
...
### 1.2. Блок E (Step 24–28) — инженерный фундамент C++

#### WBS-2.1 — Step 24: Минимально собираемый C++‑скелет + CLI поверхность (заглушки)
...
#### WBS-2.3 — Step 26: CI кроссплатформенный (Windows/Linux/macOS)
...
#### WBS-2.4 — Step 27: Harness сравнения (Rust vs C++) по golden runs
- Результат: воспроизводимый diff‑harness, отчёт совпадений/расхождений по `RUN-*` внутри платформы.
- Зависимости: WBS-2.1, WBS-2.3; эталоны Step 13.
- Оценка: **6 ч/д** (High).
- Трассируемость:
...
#### WBS-2.5 — Step 28: Политика тестовых данных (EVTX/Sigma/фикстуры)
...
### 1.3. Блок F (Step 29–30) — подготовка цикла портирования

#### WBS-3.1 — Step 29: Портинг‑бэклог (вертикальные слайсы)
...
- Трассируемость:
  ...
  - RUN: RUN-0001…RUN-0601.
  - RISK: RISK-0011, RISK-0030, RISK-0031, RISK-0032 (как факторы гранулярности).

#### WBS-3.2 — Step 30: Iteration playbook (шаблон выходов одной итерации)
...
### 1.4. Блок F (Step 31–33) — реализация вертикальных слайсов (обобщённая WBS)

> Каждый из элементов WBS-4.* предполагает повторяющийся цикл: Step 31 (UnitReady), Step 32 (Unit implementation), Step 33 (UnitDone verification).
...
- Трассируемость:
  ...
  - RUN: RUN-0201…RUN-0204, RUN-0301…RUN-0303, RUN-0401…RUN-0402.
  - RISK: RISK-0030.
...
- Трассируемость:
  ...
  - FEAT: FEAT-0006.
  - MOD: MOD-0013.
  - RUN: RUN-0103…RUN-0104, RUN-0601.
...
### 1.5. Блок G (Step 34–44) — закрытие gaps, hardening и релизная готовность

#### WBS-5.1 — Step 34: Gap analysis parity (REQ/FEAT/TST/RUN)
...
#### WBS-5.7 — Step 40: Packaging / дистрибутив (кроссплатформенно)
...
#### WBS-5.8 — Step 41: Cross‑platform e2e validator
...
|---|---|---:|
| Блок D: завершение To‑Be (Step 21–23) | WBS-1.1…WBS-1.3 | 7 |
| Блок E: инженерный фундамент (Step 24–28) | WBS-2.1…WBS-2.5 | 22 |
| Блок F: подготовка цикла (Step 29–30) | WBS-3.1…WBS-3.2 | 5 |
| Блок F: реализация слайсов (Step 31–33, суммарно) | WBS-4.1…WBS-4.18 | 198 |
| Блок G: закрытие/hardening (Step 34–44) | WBS-5.1…WBS-5.11 | 73 |
| **Итого Base (P50)** |  | **305** |
...
1) WBS-4.* — это «обобщённый план». Фактическая реализация пойдёт через Step 29–33 как **слайсы**, которые при необходимости будут дробиться по правилам S1–S4.
2) Приоритет слайсов должен следовать правилам Step 29 (ценность для harness/снижение неопределённости/платформенные риски/«много тестов — мало изменений»).
