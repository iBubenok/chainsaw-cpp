...
- Версия: 1
- Статус: Draft (As‑Is)
- Назначение: зафиксировать полный список тестов upstream Chainsaw и их семантику как основу **test‑to‑test** переноса (Step 32+).
- Single Source of Truth для `TST-*` в рамках Step 12.
...
Не включено:
- эталонные прогоны бинаря (golden runs) — это Step 13;
...
- Фикстура `tests/srum/SOFTWARE` (~74MB) и большой expected JSON (`analysis_srum_database_json.txt` ~3.5MB) влияют на размер репозитория/артефактов и время CI. Политика хранения и минимизация набора данных — Step 28 (RISK-0003).
