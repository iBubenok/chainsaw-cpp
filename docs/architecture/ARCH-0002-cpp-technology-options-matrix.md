# ARCH-0002 — Матрица вариантов технологий/зависимостей для C++ (Step 17)

## Статус
- **Назначение документа:** фиксация вариантов (options) и критериев выбора. **Не является решением**.
- **Решения (выбор конкретных библиотек/подходов)** оформляются в Step 18 как `ADR-*`.

## Цель
Собрать и структурировать варианты технологий/зависимостей для C++ порта (Windows/Linux/macOS), чтобы в Step 18 можно было принять решения, не нарушая инварианты 1:1 и требования безопасности/эксплуатации.
...
> Критерии выводятся из `REQ-*` и `POL-*`; конкретные библиотеки будут выбраны в Step 18.
...
## План дальнейших действий (Step 18)
...
2) После ADR перейти к To‑Be архитектуре (Step 19).

---

## Справка: источники лицензий/статусов (для Step 18)
> Ссылки приведены для удобства верификации; окончательная проверка совместимости с GPLv3 и включения NOTICE выполняется в Step 18–19.
